...
public class PayControllerTest {
    ...
    public void integrationTest() {
        ...
        URI createCustomerUri = UriComponentsBuilder.fromHttpUrl(url).path("createCustomer")
                .queryParam("id", customerId).build().toUri();
        this.restTemplate.put(createCustomerUri, null);

        URI createRestaurantUri = UriComponentsBuilder.fromHttpUrl(url).path("createRestaurant").
                queryParam("id", restaurantId).build().toUri();
        this.restTemplate.put(createRestaurantUri, null);

        URI getCustomerBalanceUri = UriComponentsBuilder.fromHttpUrl(url).path("getCustomerBalance")
                .queryParam("id", customerId).build().toUri();
        String custBalance = this.restTemplate.getForObject(getCustomerBalanceUri, String.class);
        assertEquals("0.0", custBalance);

        URI getRestaurantBalanceUri = UriComponentsBuilder.fromHttpUrl(url).path("getRestaurantBalance")
                .queryParam("id", restaurantId).build().toUri();
        ...
        URI topUpUri = UriComponentsBuilder.fromHttpUrl(url).path("doTopup").build().toUri();
        ...
        URI transferUri = UriComponentsBuilder.fromHttpUrl(url).path("doTransfer").build().toUri();
    ...
    @Test
    public void integrationTest_daliyLimitBreach() {
